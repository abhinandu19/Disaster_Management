...
#PREDICTION---------------------------------------------------
def Prediction():
    global Testdata, cat_clf, cat_reg, cat_reg
    ...
    text.insert(END, str(Testdata.head()) + "\n\n")
    ...
    Testdata["pred_response_efficiency_score"] = pred_class
...
# ============== BACKGROUND IMAGE ===============================
bg_image = Image.open("disasters_image3.jpeg").resize((1400, 900))
...
TrainTestSplittingButton.config(bg='gold',fg='black')
...
DecisionTreeButton.config(bg='navyblue',fg='white')
...
sgdButton.config(bg='lightgreen',fg='black')
...
CatBoostButton.config(bg='white',fg='navyblue')

PredictionButton = Button(main, text="Prediction", command=Prediction)
PredictionButton.place(x=50,y=500)
PredictionButton.config(font=ff)
PredictionButton.config(bg='yellow',fg='black')
